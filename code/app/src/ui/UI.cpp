...
namespace NRenderer
{
    UI::UI() : width(1600), height(900), title("服务于本科教学的三维渲染系统"), window(nullptr), uiContext(), views() {}
    ...
    UI::~UI()
    {
        for (auto pView : views) { delete pView; }
    }
    ...
    void UI::init()
    {
        ...
        if (!gladLoadGLLoader((GLADloadproc)glfwGetProcAddress)) throw "GLAD fail to initialize";
...
#pragma endregion _INIT_
    ...
    void UI::run()
    {
        ...
        io.Fonts->AddFontFromFileTTF(
            "c:/windows/fonts/msyh.ttc", 16.0f, NULL, io.Fonts->GetGlyphRangesChineseSimplifiedCommon());
        (void)io;
...
#pragma region IMGUI_RENDER
            ...
            for (auto view : views) { view->display(); }
