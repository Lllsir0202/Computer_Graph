...
#pragma region __SHADER_SOURCE__
    const char* nodeVShaderSource  = "#version 330 core                                                  \n"
                                     ...
                                     "}                                                                  \n";
    const char* nodeFShaderSource  = "#version 330 core                                                  \n"
                                     "out vec4 FragColor;                                                \n"
    ...
    const char* lightVShaderSource = "#version 330 core                                                  \n"
                                     ...
                                     "}                                                                  \n";
    const char* lightFShaderSource = "#version 330 core                                                  \n"
                                     "out vec4 FragColor;                                                \n"
                                     "uniform vec4 drawColor;                                            \n"
                                     "void main()                                                        \n"
                                     "{                                                                  \n"
...
#pragma endregion __SHADER_SOURCE__
    ...
    float ScreenView::getShrinkNum() { return 1.f - float(shrinkLevel) * 0.125; }
    ...
    void ScreenView::draw()
    {
        ...
        ImGui::BeginChild(
            "Screen Content", size, false, ImGuiWindowFlags_HorizontalScrollbar | ImGuiWindowFlags_NoMove);
        {
            if (viewType == ViewType::PREVIEW) { preview(); }
            else if (viewType == ViewType::RESULT) { result(); }
        }
        ImGui::EndChild();
        ImGui::Separator();
        ...
        ImGui::SameLine();
        const string shrinkLevelsLabel[8] = {"100.0%", "87.5%", "75.0%", "62.5%", "50.0%", "37.5%", "25.0%", "12.5%"};
        ...
        if (ImGui::BeginCombo("##ShrinkLevel", shrinkLevelsLabel[shrinkLevel].c_str()))
        {
            for (int i = 0; i < 8; i++)
            {
                ...
                if (ImGui::Selectable(shrinkLevelsLabel[i].c_str(), &selected)) { shrinkLevel = i; }
            }
            ImGui::EndCombo();
        }
        if (viewType == ViewType::PREVIEW)
    ...
    void ScreenView::align(const Vec2& size)
    {
        ...
        if (size.x < x) { ImGui::SetCursorPosX((x - size.x) / 2); }
        if (size.y < y) { ImGui::SetCursorPosY((y - size.y) / 2); }
    }
