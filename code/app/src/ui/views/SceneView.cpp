...
namespace NRenderer
{
    ...
    void SceneView::drawBeginWindow() { ImGui::Begin("Render Settings", nullptr, windowFlag); }
    void SceneView::draw()
    {
        cameraSetting();
        ImGui::Separator();
    ...
    void SceneView::renderSetting()
    {
        ...
        ImGui::InputScalar("Photon Nums", ImGuiDataType_U32, &rs.PhotonNums, &intStep, NULL, "%u");
        ImGui::InputScalar("Sample photons", ImGuiDataType_U32, &rs.PhotonIters, &intStep, NULL, "%u");
        ImGui::Selectable("If caustic", &rs.ifcaustic);
    }
    void SceneView::ambientSetting()
    {
        ...
        if (curr == 0) { ImGui::ColorEdit3("Ambient", &as.ambient.r, ImGuiColorEditFlags_Float); }
        else if (curr == 1)
        {
    ...
    void SceneView::componentSetting()
    {
        ...
        if (ImGui::BeginCombo("Render Component##SceneView", comboPre.c_str()))
        {
            for (int i = 0; i < components.size(); i++)
            {
                ...
                if (ImGui::Selectable(
                        (to_string(i + 1) + ". " + components[i].name + "##SceneView").c_str(), &selected))
                {
        ...
        if (ImGui::Button("Render"))
        {
            ...
            else { getServer().logger.error("No render component is selected!"); }
        }
    }
