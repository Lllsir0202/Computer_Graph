    ...
    void AssetView::tempPropEditor()
    {
        ...
        if (ImGui::BeginCombo("Property Type", types[tempPropTypeIndex].c_str()))
        {
            for (int i = 0; i < 7; i++)
            {
                ...
                if (ImGui::Selectable(types[i].c_str(), &selected)) { tempPropTypeIndex = i; }
            }
            ImGui::EndCombo();
        }
...
#define SET(__T__, __V__) \
    tempProp.valueWrapper = Property::Wrapper::__T__##Type { __V__ }
        ...
        else if (tempPropTypeIndex == 6)
        {
            ...
            if (ImGui::BeginCombo(
                    "Texture##Tex", v.valid() ? manager.assetManager.asset.textureItems[v.index()].name.c_str() : ""))
            {
...
#pragma endregion TEMP_PROP_EDITOR
    ...
    void AssetView::drawBeginWindow() { ImGui::Begin("Asset", nullptr, windowFlag | ImGuiWindowFlags_MenuBar); }
    ...
    void AssetView::menu()
    {
        if (ImGui::BeginMenuBar())
        {
            ...
            {
                if (ImGui::MenuItem("场景/模型")) { manager.assetManager.importScene(); }
                if (ImGui::MenuItem("纹理")) { manager.assetManager.importTexture(); }
                ImGui::EndMenu();
            }
            if (ImGui::BeginMenu("重置"))
            {
                if (ImGui::MenuItem("重置全部")) { manager.assetManager.clearAll(); }
                ImGui::Separator();
                if (ImGui::MenuItem("重置灯光")) { manager.assetManager.clearLight(); }
                if (ImGui::MenuItem("重置模型")) { manager.assetManager.clearModel(); }
                ImGui::EndMenu();
            }
            ImGui::EndMenuBar();
        }
    ...
    void AssetView::modelTab()
    {
        ...
        {
            const auto flags = ImGuiTableFlags_BordersInnerV | ImGuiTableFlags_BordersInnerH |
                ...
                for (int i = 0; i < mis.size(); i++)
                {
                    ...
                    if (model_selected)
                    {
                        ...
                        for (auto nIdx : mi.model->nodes)
                        {
                            ...
                            if (ImGui::Selectable((to_string(nIdx + 1) + "##NodeItemIndex").c_str(),
                                    &node_selected,
                                    ImGuiSelectableFlags_SpanAllColumns))
                            {
        ...
        ImGui::BeginChild("ModelSetting");
        {
            ...
            if (uiContext.previewMode == UIContext::PreviewMode::PREVIEW_MODEL && uiContext.previewModel >= 0 &&
                uiContext.previewModel < mis.size())
            {
                ...
                ImGui::Separator();
                ImGui::DragFloat3(("Translation##ModelSelected" + to_string(uiContext.previewModel)).c_str(),
                    &mi.model->translation.x,
                    0.5,
                    0,
                    0);
                ImGui::DragFloat3(("Scale##ModelSelected" + to_string(uiContext.previewModel)).c_str(),
                    &mi.model->scale.x,
                    0.05,
                    0,
                    0);
            }
            else if (uiContext.previewMode == UIContext::PreviewMode::PREVIEW_NODE && uiContext.previewNode >= 0 &&
                     uiContext.previewNode < nis.size())
            {
                ...
                auto  selectMaterial = [&mtls](Handle& mtlHandle) -> void {
                    ...
                    if (ImGui::BeginCombo("Material##SphereNode", comboPreview.c_str()))
                    {
                        for (int i = 0; i < mtls.size(); i++)
                        {
                            ...
                            auto& mtl      = mtls[i];
                            if (ImGui::Selectable(
                                    (to_string(i + 1) + ". " + mtl.name + "##NodeSettings").c_str(), &selected))
                            {
                ...
                if (change) { asset.updateNodeGlDrawData(ni); }
            }
        }
        ImGui::EndChild();
        ImGui::Columns(1);
...
#define SET(__T__, __V__) \
    p.valueWrapper = W::__T__##Type { __V__ }
        ...
        for (auto& p : spMaterial->properties)
        {
            ...
            if (ImGui::Button(" - ")) { spMaterial->removeProperty(p.key); }
            ...
            switch (p.type)
            {
                ...
                case T::TEXTURE_ID:
                {
                    ...
                    if (ImGui::BeginCombo((p.key + "##PETex" + to_string(i)).c_str(),
                            v.valid() ? manager.assetManager.asset.textureItems[v.index()].name.c_str() : ""))
                    {
                        for (int i = 0; i < manager.assetManager.asset.textureItems.size(); i++)
                        {
                            bool selected = v.valid() ? (i == v.index()) : false;
...
#undef SET
    ...
    void AssetView::materialTab()
    {
        ...
        if (ImGui::BeginPopupModal("Add Material", NULL, ImGuiWindowFlags_AlwaysAutoResize))
        {
            ...
            if (ImGui::InputText("Material Name", buf, 64)) { tempMaterialItem.name = string(buf); }
            ...
            if (ImGui::Button("Add property")) { ImGui::OpenPopup("Property Editor"); }
            ImVec2 center(ImGui::GetIO().DisplaySize.x * 0.5f, ImGui::GetIO().DisplaySize.y * 0.5f);
            ImGui::SetNextWindowPos(center, ImGuiCond_Appearing, ImVec2(0.5f, 0.5f));
            ...
            if (ImGui::Button("Templates")) { ImGui::OpenPopup("Select a template"); }
            ...
            ImGui::SameLine();
            if (ImGui::Button("Cancel##Add_Material")) { ImGui::CloseCurrentPopup(); }
            ImGui::EndPopup();
        }
        ...
        ImGui::BeginChild("MaterialSetting");
        {
            if (currMtlIndex < mtlItems.size())
            {
                ...
                if (ImGui::InputText("Name##MTLSelected", buf, 64)) { mtlItem.name = string(buf); }
                ...
                if (it == MaterialTemplates::templates.end()) { info = "Custom Material"; }
                else { info = it->second.key; }
                ImGui::TextUnformatted(info.c_str());
                if (ImGui::Button("Add Property...")) { ImGui::OpenPopup("Property Editor##2"); }
                ImGui::SetNextWindowPos(center, ImGuiCond_Appearing, ImVec2(0.5f, 0.5f));
    ...
    void AssetView::lightTab()
    {
        ...
        if (currLightIndex != -1 && currLightIndex < lights.size())
        {
            ...
            if (l.type == T::POINT)
            {
                ...
                ImGui::DragFloat3(
                    ("Intensity##LightSelected" + to_string(currLightIndex)).c_str(), &p.intensity[0], 0.01);
            ...
            else if (l.type == T::AREA)
            {
                ...
                ImGui::DragFloat3(
                    ("Intensity##LightSelected" + to_string(currLightIndex)).c_str(), &a.radiance[0], 0.01);
                ...
                if (dirty) { manager.assetManager.asset.updateLightGlDrawData(li); }
            }
            else if (l.type == T::DIRECTIONAL)
            {
                auto& d = *manager.assetManager.asset.directionalLights[l.entity];
                ImGui::DragFloat3(
                    ("Irradiance##LightSelected" + to_string(currLightIndex)).c_str(), &d.irradiance[0], 0.01);
                ImGui::DragFloat3(
                    ("Direction##LightSelected" + to_string(currLightIndex)).c_str(), &d.direction[0], 0.1);
            }
            else if (l.type == T::SPOT)
            {
                auto& s = *manager.assetManager.asset.spotLights[l.entity];
                ImGui::DragFloat3(
                    ("Intensity##LightSelected" + to_string(currLightIndex)).c_str(), &s.intensity[0], 0.01);
                ImGui::DragFloat3(("Position##LightSelected" + to_string(currLightIndex)).c_str(), &s.position[0], 1);
                ImGui::DragFloat3(
                    ("Direction##LightSelected" + to_string(currLightIndex)).c_str(), &s.direction[0], 0.1);
