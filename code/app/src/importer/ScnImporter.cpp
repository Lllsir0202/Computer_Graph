...
namespace NRenderer
{
    bool ScnImporter::parseMtl(Asset& asset, ifstream& file, map<string, size_t>& mtlMap)
    {
        ...
        while (getline(file, currline))
        {
            ...
            else if (token == "Material")
            {
                ...
                if (mtlMap.find(name) == mtlMap.end()) { mtlMap[name] = index; }
                else
                {
    ...
    bool ScnImporter::import(Asset& asset, const string& path)
    {
        ...
        while (getline(file, currline))
        {
            ...
            {
                ss >> token;
                if (token == "Material") { successFlag = parseMtl(asset, file, mtlMap); }
                else if (token == "Model") { successFlag = parseMdl(asset, file, mtlMap); }
                else if (token == "Light") { successFlag = parseLgt(asset, file); }
