...
namespace NRenderer
{
    ...
    {
        for (auto& mi : asset.materialItems) { this->scene->materials.push_back(*mi.material); }
        for (auto& ti : asset.textureItems) { this->scene->textures.push_back(*ti.texture); }
        for (auto& mi : asset.modelItems) { this->scene->models.push_back(*mi.model); }
        ...
        for (auto& li : asset.lightItems) { this->scene->lights.push_back(*li.light); }
        for (auto& s : asset.spheres) { this->scene->sphereBuffer.push_back(*s); }
        for (auto& t : asset.triangles) { this->scene->triangleBuffer.push_back(*t); }
        for (auto& p : asset.planes) { this->scene->planeBuffer.push_back(*p); }
        for (auto& m : asset.meshes) { this->scene->meshBuffer.push_back(*m); }
        for (auto& p : asset.pointLights) { this->scene->pointLightBuffer.push_back(*p); }
        for (auto& a : asset.areaLights) { this->scene->areaLightBuffer.push_back(*a); }
        for (auto& d : asset.directionalLights) { this->scene->directionalLightBuffer.push_back(*d); }
        for (auto& s : asset.spotLights) { this->scene->spotLightBuffer.push_back(*s); }
    }

    void SceneBuilder::buildCamera() { this->scene->camera = this->camera; }
    ...
    void SceneBuilder::buildAmbient()
    {
        ...
        else { this->scene->ambient.type = Ambient::Type::ENVIROMENT_MAP; }
    }
