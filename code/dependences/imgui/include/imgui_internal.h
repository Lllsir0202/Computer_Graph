...
struct IMGUI_API ImDrawListSharedData
{
    ...
    ImVec2 ArcFastVtx[12 * IM_DRAWLIST_ARCFAST_TESSELLATION_MULTIPLIER];  // FIXME: Bake rounded corners fill/borders in
                                                                          // atlas
    ImU8 CircleSegmentCounts[64];  // Precomputed segment count for given radius (array index + 1) before we calculate
...
struct ImGuiContext
{
    ...
    ImGuiWindow* HoveredRootWindow;  // == HoveredWindow ? HoveredWindow->RootWindow : NULL, merely a shortcut to avoid
                                     // null test in some situation.
    ...
    ImVector<ImGuiID> FocusScopeStack;  // Stack for PushFocusScope()/PopFocusScope() - not inherited by Begin(), unless
                                        // child window
    ...
    ImDrawList ForegroundDrawList;  // Last draw list to be rendered. This is where we the render software mouse cursor
                                    // (if io.MouseDrawCursor is set) and most debug overlays.
...
struct IMGUI_API ImGuiWindowTempData
{
    ...
    ImVec2 CursorMaxPos;    // Used to implicitly calculate ContentSize at the beginning of next frame. Always growing
                            // during the frame.
...
// FIXME-TABLE: transient data could be stored in a per-stacked table structure: DrawSplitter, SortSpecs, incoming
// RowData
struct ImGuiTable
{
    ImGuiID                  ID;
    ...
    ImDrawListSplitter DrawSplitter;  // We carry our own ImDrawList splitter to allow recursion (FIXME: could be stored
                                      // outside, worst case we need 1 splitter per recursing table)
    ...
    ImGuiTableDrawChannelIdx Bg2DrawChannelCurrent;  // For Selectable() and other widgets drawing accross columns after
    ...
    bool IsOuterRectMinFitX;  // Set when outer_size.x == 0.0f in BeginTable(), scrolling is disabled, and there are no
                              // stretch columns.
...
namespace ImGui
{
    ...
    IMGUI_API void Shutdown(
        ImGuiContext* context);  // Since 1.60 this is a _private_ function. You can call DestroyContext() to destroy
    ...
    IMGUI_API void SetLastItemData(
        ImGuiWindow* window, ImGuiID item_id, ImGuiItemStatusFlags status_flags, const ImRect& item_rect);
    ...
    IMGUI_API void RenderColorRectWithAlphaCheckerboard(ImDrawList* draw_list, ImVec2 p_min, ImVec2 p_max,
        ImU32 fill_col, float grid_step, ImVec2 grid_off, float rounding = 0.0f, int rounding_corners_flags = ~0);
    ...
    IMGUI_API void DebugNodeDrawCmdShowMeshAndBoundingBox(
        ImGuiWindow* window, const ImDrawList* draw_list, const ImDrawCmd* draw_cmd, bool show_mesh, bool show_aabb);
...
IMGUI_API void ImFontAtlasBuildRender1bppRectFromString(ImFontAtlas* atlas, int atlas_x, int atlas_y, int w, int h,
