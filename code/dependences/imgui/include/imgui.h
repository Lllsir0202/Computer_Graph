...
typedef int ImDrawCornerFlags;   // -> enum ImDrawCornerFlags_    // Flags: for ImDrawList::AddRect(), AddRectFilled()
                                 // etc.
...
namespace ImGui
{
    ...
    // to share a font atlas between imgui contexts. None of those functions is reliant on the current context.
    ...
    IMGUI_API void SetWindowFocus();  // (not recommended) set current window to be focused / top-most. prefer using
                                      // SetNextWindowFocus().
    IMGUI_API void SetWindowFontScale(float scale);  // set font scale. Adjust IO.FontGlobalScale if you want to scale
    ...
    IMGUI_API void PushStyleVar(
        ImGuiStyleVar idx, const ImVec2& val);  // modify a style ImVec2 variable. always use this if you modify the
                                                // style after NewFrame().
    IMGUI_API void PopStyleVar(int count = 1);
    IMGUI_API void PushAllowKeyboardFocus(
        bool allow_keyboard_focus);  // allow focusing using TAB/Shift-TAB, enabled by default but you can disable it
                                     // for certain widgets
    ...
    IMGUI_API void PushItemWidth(
        float item_width);  // push width of items for common large "item+label" widgets. >0.0f: width in pixels, <0.0f
                            // align xx pixels to the right of window (so -FLT_MIN always align width to the right
                            // side). 0.0f = default to ~2/3 of windows width,
    ...
    IMGUI_API bool TreeNode(const char* str_id, const char* fmt, ...) IM_FMTARGS(
        2);  // helper variation to easily decorelate the id from the displayed string. Read the FAQ about why and how
    ...
    IMGUI_API bool Selectable(const char* label, bool selected = false, ImGuiSelectableFlags flags = 0,
        const ImVec2& size = ImVec2(
            0, 0));  // "bool selected" carry the selection state (read-only). Selectable() is clicked is returns true
    ...
    IMGUI_API bool ListBoxHeader(const char* label,
        const ImVec2&                        size = ImVec2(
            0, 0));  // use if you want to reimplement ListBox() will custom data or interactions. if the function
                     // return true, you can output elements then call ListBoxFooter() afterwards.
    IMGUI_API bool ListBoxHeader(const char* label, int items_count, int height_in_items = -1);  // "
    IMGUI_API void ListBoxFooter();  // terminate the scrolling region. only call ListBoxFooter() if ListBoxHeader()
                                     // returned true!
    ...
    IMGUI_API bool BeginPopupContextItem(const char* str_id = NULL,
        ImGuiPopupFlags                              popup_flags =
            1);  // open+begin popup when clicked on last item. if you can pass a NULL str_id only if the previous item
                 // had an id. If you want to use that on a non-interactive item such as Text() you need to pass in an
    ...
    // - 4. Optionally call TableHeadersRow() to submit a header row. Names will be pulled from data provided
    ...
    IMGUI_API bool BeginTable(const char* str_id, int columns_count, ImGuiTableFlags flags = 0,
        const ImVec2& outer_size = ImVec2(-FLT_MIN, 0), float inner_width = 0.0f);
    ...
    //   Important: this will not display anything! The name passed to TableSetupColumn() is used by TableHeadersRow()
    //   and context-menus.
    // - Use TableHeadersRow() to create a row and automatically submit a TableHeader() for each column.
    //   Headers are required to perform: reordering, sorting, and opening the context menu (but context menu can also
    //   be available in columns body using ImGuiTableFlags_ContextMenuInBody).
    // - You may manually submit headers using TableNextRow() + TableHeader() calls, but this is only useful in some
    // advanced cases (e.g. adding custom widgets in header row).
    // - Use TableSetupScrollFreeze() to lock columns (from the right) or rows (from the top) so they stay visible when
    // scrolled.
    IMGUI_API void TableSetupColumn(
        const char* label, ImGuiTableColumnFlags flags = 0, float init_width_or_weight = 0.0f, ImU32 user_id = 0);
    ...
    // - Most functions taking 'int column_n' treat the default value of -1 as the same as passing the current column
    // index
    // - Sorting: call TableGetSortSpecs() to retrieve latest sort specs for the table. Return value will be NULL if no
    // sorting.
    //   When 'SpecsDirty == true' you should sort your data. It will be true when sorting specs have changed since last
    //   call, or the first time. Make sure to set 'SpecsDirty = false' after sorting, else you may wastefully sort your
    //   data every frame! Lifetime: don't hold on this pointer over multiple frames or past any subsequent call to
    //   BeginTable().
    ...
    IMGUI_API const ImGuiPayload* AcceptDragDropPayload(const char* type,
        ImGuiDragDropFlags flags = 0);   // accept contents of a given type. If ImGuiDragDropFlags_AcceptBeforeDelivery
    ...
    IMGUI_API bool IsItemDeactivated();  // was the last item just made inactive (item was previously active). Useful
                                         // for Undo/Redo patterns with widgets that requires continuous editing.
    IMGUI_API bool IsItemDeactivatedAfterEdit();  // was the last item just made inactive and made a value change when
    ...
    IMGUI_API void CaptureKeyboardFromApp(
        bool want_capture_keyboard_value =
            true);  // attention: misleading name! manually override io.WantCaptureKeyboard flag next frame (said flag
                    // is entirely left for your application to handle). e.g. force capture keyboard when your widget is
    ...
    IMGUI_API ImVec2 GetMouseDragDelta(ImGuiMouseButton button = 0,
        float                                           lock_threshold =
            -1.0f);  // return the delta from the initial clicking position while the mouse button is pressed or was
                     ...
                     // least once (if lock_threshold < -1.0f, uses io.MouseDraggingThreshold)
    ...
    IMGUI_API void SaveIniSettingsToDisk(
        const char* ini_filename);  // this is automatically called (if io.IniFilename is not empty) a few seconds after
                                    // any modification that should be reflected in the .ini file (and also by
                                    // DestroyContext).
...
enum ImGuiTableFlags_
{
    ...
    ImGuiTableFlags_SizingStretchSame = 4 << 13,  // Columns default to _WidthStretch with default weights all equal,
                                                  // unless overriden by TableSetupColumn(). Sizing Extra Options
    ImGuiTableFlags_NoHostExtendY =
        1 << 16,  // Disable extending table past the limit set by outer_size.y. Only meaningful when neither ScrollX
                  // nor ScrollY are set (data below the limit will be clipped and not visible)
    ImGuiTableFlags_NoKeepColumnsVisible =
        1 << 17,  // Disable keeping column always minimally visible when ScrollX is off and table gets too small. Not
                  // recommended if columns are resizable.
    ImGuiTableFlags_PreciseWidths =
        1 << 18,  // Disable distributing remainder width to stretched columns (width allocation on a 100-wide table
    ...
    ImGuiTableFlags_NoClip =
        1 << 19,  // Disable clipping rectangle for every individual columns (reduce draw command count, items will be
                  // able to overflow into other columns). Generally incompatible with TableSetupScrollFreeze(). Padding
    ImGuiTableFlags_PadOuterX =
        1 << 20,  // Default if BordersOuterV is on. Enable outer-most padding. Generally desirable if you have headers.
    ImGuiTableFlags_NoPadOuterX = 1 << 21,  // Default if BordersOuterV is off. Disable outer-most padding.
    ImGuiTableFlags_NoPadInnerX =
        1 << 22,  // Disable inner padding between columns (double inner padding if BordersOuterV is on, single inner
                  // padding if BordersOuterV is off). Scrolling
    ImGuiTableFlags_ScrollX =
        1 << 23,  // Enable horizontal scrolling. Require 'outer_size' parameter of BeginTable() to specify the
    ...
    ImGuiTableFlags_ScrollY = 1 << 24,  // Enable vertical scrolling. Require 'outer_size' parameter of BeginTable() to
                                        // specify the container size. Sorting
    ImGuiTableFlags_SortMulti = 1 << 25,     // Hold shift when clicking headers to sort on multiple column.
                                             // TableGetSortSpecs() may return specs where (SpecsCount > 1).
    ImGuiTableFlags_SortTristate = 1 << 26,  // Allow no sorting, disable default sorting. TableGetSortSpecs() may
...
enum ImGuiTableColumnFlags_
{
    ...
    ImGuiTableColumnFlags_WidthStretch =
        1 << 2,  // Column will stretch. Preferable with horizontal scrolling disabled (default if table sizing policy
    ...
    ImGuiTableColumnFlags_IndentDisable = 1
                                          << 16,  // Ignore current Indent value when entering cell (default for columns
                                                  // > 0). Indentation changes _within_ the cell will still be honored.

    // Output status flags, read-only via TableGetColumnFlags()
    ImGuiTableColumnFlags_IsEnabled = 1 << 20,  // Status: is enabled == not hidden by user/api (referred to as "Hide"
