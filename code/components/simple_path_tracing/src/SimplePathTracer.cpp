...
{
    RGB SimplePathTracerRenderer::gamma(const RGB& rgb) { return glm::sqrt(rgb); }
    ...
    auto SimplePathTracerRenderer::render() -> RenderResult
    {
        ...
        {
            t[i] = thread(&SimplePathTracerRenderer::renderTask, this, pixels, width, height, i, taskNums);
        }
