...
namespace PhotonMapping
{
    ...
    AABB::AABB(Triangle* tr) : entity(tr), isLeaf(true), type(T::TRIANGLE), left(left), right(right)
    {
        Vec3& v1 = tr->v[0];
        Vec3& v2 = tr->v[1];
        Vec3& v3 = tr->v[2];
    ...
    AABB::AABB(Plane* pl) : entity(pl), isLeaf(true), type(T::PLANE), left(left), right(right) 
    {
        ...
        minp = {
            min(c1[0], min(c2[0], min(c3[0], c4[0]))),
            min(c1[1], min(c2[1], min(c3[1], c4[1]))),
            min(c1[2], min(c2[2], min(c3[2], c4[2])))
        };
        maxp = {
            max(c1[0], max(c2[0], max(c3[0], c4[0]))),
    ...
    { 
        ...
        float near   = max(tNear[0], max(tNear[1], tNear[2]));
        float far    = min(tFar[0], min(tFar[1], tFar[2]));
