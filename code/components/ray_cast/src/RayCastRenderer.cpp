...
namespace RayCast
{
    ...
    RGB  RayCastRenderer::gamma(const RGB& rgb) { return glm::sqrt(rgb); }
    auto RayCastRenderer::render() -> RenderResult
    {
        ...
        for (auto& mtl : scene.materials) { shaderPrograms.push_back(shaderCreator.create(mtl, scene.textures)); }
    ...
    RGB RayCastRenderer::trace(const Ray& r)
    {
        ...
        if (closestHitObj)
        {
            ...
            if (glm::dot(out, hitRec.normal) < 0) { return {0, 0, 0}; }
            ...
            if ((!shadowHit) || (shadowHit && shadowHit->t > distance)) { return c * l.intensity; }
            else { return Vec3{0}; }
        }
        else { return {0, 0, 0}; }
    }
