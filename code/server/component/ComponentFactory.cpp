...
namespace NRenderer
{
    bool ComponentFactory::registerComponent(
        const string& type, const string& name, const string& desc, function<SharedInstance()> constructor)
    {
    ...
    {
        auto& ncMap = constructors[type];
        if (ncMap.find(name) != ncMap.end()) { return ncMap[name].constructor(); }
        else { return nullptr; }
    }
    ...
    vector<ComponentInfo> ComponentFactory::getComponentsInfo(const string& type)
    {
        ...
        if (type == "")
        {
            ...
            {
                for (auto& it : ncMap.second) { infos.push_back(it.second.info); }
            }
        }
        else
        {
            ...
            {
                for (auto& it : ncMapIt->second) { infos.push_back(it.second.info); }
            }
        }
